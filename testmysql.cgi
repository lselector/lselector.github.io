...
use CGI;
my $q = new CGI;
my $sql = $q->param("sql") || "select * from wp_posts";
my $qf = $q->param("qf")|| 0;
print $q->header("text/html"), $q->start_html("Welcome");

my $dbh = DBI->connect("DBI:mysql:database=debt;host=localhost", "ketevan", "igwt1945", {'RaiseError' => 1} )
...
sub db_form_example {
  ...
  print $q->start_form(-name=>'form1', -method=>'POST', -action=>$self_cgi);
  print $q->p("Enter select SQL statement:");
  print $q->radio_group( -name=>"qf", -values=>["query","do"], -default=>"query",
           -labels=>{ query => "Query", do => "Just run sql" } ),br,nn;
  print $q->textarea(-name=>"sql", -rows=>'10', -columns=>'60', -default=>"$sql\n"),nn,br;
  print $q->submit, $q->reset,nn, br,nn;
