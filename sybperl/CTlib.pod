<--$Id: sybperl.pod,v 1.35 1999/09/21 21:10:34 mpeppler Exp $-->
...
explicitly in a single script is not garanteed to work correctly.
...
Returns TRUE if the B<DBPROCESS> has been marked I<DEAD> by I<DBlibrary>.
...
information returned by DBlibrary's I<dbalt*()> calls. The $column is the 
...
see the documentation of the I<dbalt*()> calls in Sybase's DBlibrary
manual.
...
B<Sybase::DBlib::NAME> (eg B<Sybase::DBlib::STDEXIT>) Additional constants are:
...
The version of I<DBlibrary> that sybperl was built against.
...
If set, BINARY results are preceeded by '0x' in the result. B<Default:> unset.
...
See also B<Sybase::BCP> for a symplified bulk copy API.
...
I<IMAGE> columns are B<not> bound, and will not be subsequewntly
...
usefull for interleaving cursor operations (see ct_cursor() above, and
...
B<CS_INT_TYPE> if an integer value (such B<CS_NETIO> is being set or
retrieved).
...
If TRUE, convert B<NUMERIC>, B<DECIMAL> and B<MONEY> data retrieved
via ct_fetch() to character strings. This ensures that there is no 
precision loss associated with converting fixed decimal values to 
floating point representation, and is faster than using the B<UseNumeric>
and/or B<UseMoney> attributes. B<UseChar> I<overrides> the setting for
B<UseNumeric> and/or B<UseMoney>.
...
First lets see how ct_get_data() is implemented to retrieve B<TEXT>
...
Traces all SQL language commands - (ie calls to dbcmd(), ct_execute() or
ct_command().) 
...
I have a simple bug tracking system at http://gw.peppler.org/cgi-bin/bug.cgi.
...
W. Phillip Moore E<lt>F<wpm@ms.com>E<gt> for the nsql() method.

Numerous folks have contributed ideas and bug fixes for which they
have my undying thanks :-) 

The sybperl mailing list E<lt>F<sybperl-l@listproc.net>E<gt> is the
